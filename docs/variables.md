...
| `k3s_master_taint` | `false` | `true` ise config.yaml'a `node-taint`. Yalnızca yeni kaydolan node'a etki eder; worker yoksa Pending riski. Eski sürümlerde varsayılan `true` idi: o sürümle kurulmuş cluster'a master eklerken `true` yazın. |
...
| `k3s_hardening` | `true` | CIS sıkılaştırması: `secrets-encryption`, audit log, Pod Security Admission (baseline), `protect-kernel-defaults` + kubelet flag'leri, kubeconfig `0600`, PKI `*.crt` `0600`, default SA token automount kapalı, kube-system/kube-public/kube-node-lease NetworkPolicy'leri (`false` yapmak mevcut policy'leri silmez). Gerekli sysctl'leri aynı task yazar (`99-k3s-hardening.conf`). `false` → yalnızca temel server anahtarları ve eski `0644` kubeconfig. |

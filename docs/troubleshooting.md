...
| Kurulum "başarılı" ama Gateway/MetalLB/… hiç kurulmadı | bayrak `false` (varsayılan saf k3s) veya ilk master `groups['master'][0]` değil | `vars/main.yml` `*_install`; envanterde master sırası |
...
| Worker `NotReady`, pod'lar arası trafik yok, MetalLB webhook `context deadline exceeded` | RHEL/Rocky firewalld flannel VXLAN 8472/udp'yi kesiyor | `00_prerequisites.yml` açar; `add_node.yml` ile eklenen node'da açılmaz (todo A2): `firewall-cmd --add-port=8472/udp --permanent` |
| HA'da 2./3. master etcd'ye katılamıyor, keepalived'ler hepsi MASTER | firewalld 2379-2380/tcp ve VRRP kapalı (todo A3) | `firewall-cmd --add-port=2379-2380/tcp --add-protocol=vrrp --permanent --reload` |
...
| Yeni node `kubelet version newer than apiserver` / NotReady | `k3s_version: ""` ile latest geldi (todo A4) | `k3s_version`'ı `k3s --version` çıktısına pinle, node'u `k3s-*-uninstall.sh` ile sil, tekrar ekle |

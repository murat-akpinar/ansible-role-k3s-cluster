...
| add_node: `Wait for the cluster API endpoint to be reachable` zaman aşımı | 3+ master'da VIP kalkmadı (keepalived, VRRP, arayüz) | `ip -br a \| grep <vip>`, `journalctl -u keepalived --since -10m` (eski master'larda) |
...
| `*.<domain>.local` bazı istemcilerde çözülmüyor | `cluster_domain` `.local` ile verilmiş; `.local` mDNS'e ayrılı, systemd-resolved/macOS DNS sunucusuna sormaz | `resolvectl query grafana.<domain>`; `cluster_domain`'i `.home.arpa` ya da kendi zone'unuz yapın, `k3s_setup.yml` tekrar |
